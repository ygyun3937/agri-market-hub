...
export default function PestPanel() {
  ...
  return (
    <div style={{ padding: '10px 6px', marginTop: 8, borderTop: '1px solid #21262d' }}>
      ...
      {loading ? (
        <div style={{ fontSize: 11, color: '#8b949e' }}>로딩 중...</div>
      ) : filtered.length === 0 ? (
        ...
        filtered.slice(0, 5).map(p => (
          <div key={p.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
              <span style={{ fontSize: 12, fontWeight: 600, color: '#c9d1d9' }}>{p.itemName || '작물명 없음'}</span>
              {p.severity && (
            ...
            {p.region && (
              <div style={{ fontSize: 10, color: '#8b949e' }}>📍 {p.region}</div>
            )}
            {p.description && (
              <div style={{ fontSize: 11, color: '#adbac7', marginTop: 2, lineHeight: 1.4,
