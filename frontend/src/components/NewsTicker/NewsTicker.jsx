...
export default function NewsTicker() {
  ...
  useEffect(() => {
    client.get('/news?tab=all').then(res => setItems(res.data.slice(0, 10))).catch(() => {})
  }, [])
  ...
  return (
    <div style={{
      ...
      padding: '4px 16px', fontSize: 12, color: '#8b949e',
      display: 'flex', alignItems: 'center', gap: 8,
      overflow: 'hidden', flexShrink: 0
    }}>
      <span style={{ color: '#3fb950', fontWeight: 700, flexShrink: 0 }}>● LIVE</span>
      <div style={{ overflow: 'hidden', whiteSpace: 'nowrap', flex: 1 }}>
        <span style={{ display: 'inline-block', animation: 'ticker 40s linear infinite' }}>
          {text}
        </span>
      </div>
