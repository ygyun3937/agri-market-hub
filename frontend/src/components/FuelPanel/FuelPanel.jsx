    ...
    <div style={{ padding: '12px 6px', marginTop: 'auto' }}>
      ...
      {fuels.map(f => (
        <div key={f.id} style={{ display: 'flex', justifyContent: 'space-between',
          alignItems: 'center', fontSize: 14, color: '#ddeaf5', marginBottom: 8 }}>
          <span>{LABELS[f.type] || f.type}</span>
          <span style={{ fontWeight: 700, fontSize: 15, color: '#eef5fb' }}>₩{f.price?.toLocaleString()}</span>
        </div>
      ))}
      {fuels.length === 0 && (
        <div style={{ fontSize: 11, color: '#87b8d4' }}>데이터 없음</div>
      )}
    </div>
  )
}
